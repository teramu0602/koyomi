...
public function leaveGroup($groupId)
{
    ...
    if ($backUrl && str_contains($backUrl, "group_id={$groupId}")) {
        ...
        $redirectUrl = $groupExists ? $backUrl : route('calendar'); // グループがなければ個人カレンダー
    }
    // 退会対象のユーザー情報取得
    $userGroup = UserGroup::where('user_id', $userId)->where('group_id', $groupId)->first();
    ...
    // ユーザーをグループから削除
    $userGroup->delete();
    // セッションにそのグループへの戻りURLが入っていたら、存在しなければ個人カレンダーに書き換える
    $backUrl = session('group_calendar_back_url');
    if ($backUrl && str_contains($backUrl, "group_id={$groupId}")) {
        $groupExists = Group::where('id', $groupId)->exists();
        if (!$groupExists) {
    ...
    // セッションの戻り先URLがあればそれを使う。なければグループ一覧へ。
$redirectUrl = session('group_calendar_back_url', route('groups.list'));
