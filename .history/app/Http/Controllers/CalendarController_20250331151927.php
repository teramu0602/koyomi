...
//お試しコード
    public function index()
    {
        ...
        $events = Calendar::where(function ($query) use ($year, $month) {
            $query->whereYear('event_start_date', $year)
                  ->whereMonth('event_start_date', $month);
        })->orWhere(function ($query) use ($year, $month) {
            $query->whereYear('event_end_date', $year)
                  ->whereMonth('event_end_date', $month);
        })->get();
        // ビューにデータを渡す
        return view('calender.home', compact('post','events'));
    }
...
public function show($year, $month)
{
    ...
    return view('calender.home', compact('year', 'month','post','schedules','events'));
}
}
