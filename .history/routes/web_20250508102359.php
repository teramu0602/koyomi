...
Route::get('/back-to-group', function () {
    $url = session('group_calendar_back_url');

    // URLがなければ個人カレンダーにリダイレクト
    if (!$url) {
        return redirect()->route('calendar');
    }

    // グループIDをURLから取り出す（例: /group_home/{id}/... の形式を想定）
    preg_match('#/group_home/(\d+)#', $url, $matches);
    $groupId = $matches[1] ?? null;

    // グループが存在しない or IDが取れない場合 → カレンダーへ
    if (!$groupId || !\App\Models\Group::find($groupId)) {
        return redirect()->route('calendar');
    }

    // 正常なら保存してあったグループカレンダーへ
    return redirect($url);
})->name('safe.back.to.group');
