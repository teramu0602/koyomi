...
@php
    ...
    if ($backUrl) {
        // group_id=◯◯ 形式からID抽出（例: /group_home?id=3）
        if (preg_match('/group_id=(\d+)/', $backUrl, $matches)) {
            $groupId = $matches[1];
            ...
            if ($groupExists && !$isDeleted) {
                ...
                $finalUrl = route('calendar');
            }
        } else {
            // URLにgroup_idが含まれない場合 → 安全に個人カレンダー
            $finalUrl = route('calendar');
        }
    }
@endphp
...
@if ($finalUrl)
    <a href="{{ $finalUrl }}" class="btn btn-secondary">戻る</a>
@endif
