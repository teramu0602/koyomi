...
@if (session()->has('group_calendar_back_url'))
    <a href="{{ session('group_calendar_back_url') }}">戻る</a>
@endif
...
@php
    ...
    if ($backUrl && preg_match('/group_id=(\d+)/', $backUrl, $matches)) {
        $groupId = $matches[1];
        if (!\App\Models\Group::find($groupId)) {
            $isInvalid = true;
        }
    }
@endphp
...
@if ($backUrl)
    <a href="{{ $isInvalid ? route('calendar') : $backUrl }}" class="btn btn-secondary">戻る正</a>
    elseif
    <a href="{{ route('calendar') }}" class="btn btn-secondary">戻る正</a>
@endif
