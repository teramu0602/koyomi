...
<table>
    ...
    <tr>
        ...
        <td class="{{ $class }}" onclick="window.location.href='飛びたいパス';">
            ...
            <div>
                ab
                @foreach(array_filter($post, function($e){
                    //$eはイベント情報 
                    $e=event_start_date;
                    //$eのstart_dateがそのセルの年月日に一致すればtrueという式を書く
                    $e->event_start_date === sprintf('%04d-%02d-%02d', $year, $month, $day);
                }) as $event)
                    <div>{{$event->title}}</div>
                @endforeach
...
<tr>
    @foreach ($week as $day)
        @php
            ...
            // 該当日のイベント取得
            $dayEvents = $events->where('event_start_date', "$year-$month-".str_pad($day, 2, '0', STR_PAD_LEFT));
        @endphp
        <td class="{{ $class }}">
            <span>{{ $day }}</span>
            @foreach ($dayEvents as $event)
                <div class="event-box">{{ $event->title }}</div>
            @endforeach
        </td>
    @endforeach
</tr>
    ...
    <td class="{{ $class }}">
        ...
        @if (isset($schedules[$day]))
            @foreach ($schedules[$day] as $schedule)
                <div class="schedule">{{ $schedule->title }}</div>
            @endforeach
        @endif
    </td>
    @endforeach
